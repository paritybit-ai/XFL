...
class BaseTrainer(Hooker, TrainConfigParser):
    ...
    def _set_optimizer(self):
        ...
        if self.lr_scheduler:
            for k, v in optimizer_conf.items():
                optimizer[k] = get_optimizer(k, framework="paddle")(learning_rate=list(self.lr_scheduler.values())[0], parameters=self.model.parameters(), **v)
        else:
            for k, v in optimizer_conf.items():
                optimizer[k] = get_optimizer(k, framework="paddle")(parameters=self.model.parameters(), **v)
        return optimizer
    ...
    def _set_lossfunc(self):
        ...
        for k, v in loss_func_conf.items():
            loss_func[k] = get_lossfunc(k, framework="paddle")(**v)
    ...
    def _set_lr_scheduler(self):
        ...
        for (k, v) in lr_scheduler_conf.items():
            lr_scheduler[k] = get_lr_scheduler(k, framework="paddle")(**v)
        return lr_scheduler
