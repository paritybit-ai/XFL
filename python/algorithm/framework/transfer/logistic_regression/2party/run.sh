...
if [ ! -d "${PROJECT_HOME}/dataset/breast_cancer_wisconsin_transfer/2party" ]; then
  if [ ! -f "${PROJECT_HOME}/python/xfl.py" ]; then
    python "${PROJECT_HOME}/common/dataset/breast_cancer_wisconsin.py" --mode "transfer" --splits 2 --party "labeled" "1"
  else
    python "${PROJECT_HOME}/python/common/dataset/breast_cancer_wisconsin.py" --mode "transfer" --splits 2 --party "labeled" "1"
  fi
fi
