...
class VerticalDecisionTreeLabelTrainer(object):
    def __init__(self,
        ...
        # self.tree_param.loss_param.name, self.tree_param.loss_param)
        ...
        if isinstance(encryption_param, PlainParam):
            ...
            if EMBEDING:
                grad_hess = embed([self.grad[sample_index], self.hess[sample_index]], interval=(
                    1 << 128), precision=64)
    ...
    def _cal_local_best_split(self, node: Node):
        ...
        if node.sample_index is None or len(node.sample_index) == self.big_feature.data.shape[0]:
            ...
            big_feature = self.big_feature.slice_by_sample_index(
                node.sample_index)

        res_hist_list = []
        for col_name in big_feature.feature_columns:
            res_hist_list.append(
                big_feature.data.groupby([col_name])[['xfl_grad', 'xfl_hess']].agg({'sum'}))  # ({'count', 'sum'})

        for feature_idx in range(len(res_hist_list)):
            res_hist_list[feature_idx] = res_hist_list[feature_idx].cumsum(
                axis=0)
            res_hist_list[feature_idx].rename(
                columns={"sum": "cum_sum"}, inplace=True)
            cum_grad = res_hist_list[feature_idx][(
                'xfl_grad', 'cum_sum')].to_numpy()
            cum_hess = res_hist_list[feature_idx][(
                'xfl_hess', 'cum_sum')].to_numpy()

            gains = cal_gain(cum_grad, cum_hess,
                             self.tree_param.lambda_, None, None)
            ...
            max_split_index = int(res_hist_list[feature_idx][(
                'xfl_grad', 'cum_sum')].index[max_gain_index])
            ...
            if max_gain > best_split_info.gain:
                ...
                best_split_info.feature_idx = self.feature_id_mapping[feature_idx]
                best_split_info.split_point = self.split_points[feature_idx][max_split_index]

                best_split_info.left_sample_index = \
                    big_feature.data[big_feature.data.iloc[:, feature_idx + 3]
                                     <= max_split_index]['xfl_id'].tolist()
                best_split_info.right_sample_index = \
                    big_feature.data[big_feature.data.iloc[:, feature_idx + 3]
                                     > max_split_index]['xfl_id'].tolist()

                left_weight = cal_weight(cum_grad[max_gain_index],
                                         cum_hess[max_gain_index],
                                         self.tree_param.lambda_)
    ...
    def _cal_remote_best_split(self) -> Dict[str, BestSplitInfo]:
        ...
        gain_infos: Dict[str, list] = {party_id: []
                                       for party_id in self.summed_grad_hess_channs}
        is_continue_flags = np.array(
            [True for party_id in self.summed_grad_hess_channs], dtype=np.bool)
        ...
        def decrypt_hist(hist_list: List[np.ndarray], num_cores: int, out_origin: bool = True) -> list:
            ...
            hist = Paillier.decrypt(
                self.pri_context, hist, num_cores=num_cores, out_origin=out_origin)
            res = []
        ...
        while True:
            for i, party_id in enumerate(self.summed_grad_hess_channs):
                ...
                data = self.summed_grad_hess_channs[party_id].recv(
                    use_pickle=True, wait=False)
                if data is None:
                    # Data has not received, try it next round.
                    continue

                is_continue, grad_hess_hist_list = data
                ...
                if self.tree_param.encryption_param.method == PAILLIER:
                    if EMBEDING:
                        ...
                        grad_hess_hist = decrypt_hist(
                            grad_hess_hist, num_cores=self.max_num_cores, out_origin=True)

                        grad_hist_list = []
                        hess_hist_list = []
                        for hist in grad_hess_hist:
                            a, b = umbed(hist, num=2, interval=(
                                1 << 128), precison=64)
                        ...
                        grad_hist_list = decrypt_hist(
                            grad_hist_list, num_cores=self.max_num_cores, out_origin=False)
                        hess_hist_list = decrypt_hist(
                            hess_hist_list, num_cores=self.max_num_cores, out_origin=False)
                else:
                    grad_hist_list = []
                    hess_hist_list = []
                    count_hist_list = []
                ...
                for idx in range(len(grad_hess_hist_list)):
                    grad_hist, hess_hist, count_hist = grad_hist_list[
                        idx], hess_hist_list[idx], count_hist_list[idx]

                    cum_grad_hist = np.cumsum(grad_hist)
                    cum_hess_hist = np.cumsum(hess_hist)

                    gains = cal_gain(cum_grad_hist, cum_hess_hist,
                                     self.tree_param.lambda_, None, None)
                    max_gain_index = np.argmax(gains)
                    max_gain = gains[max_gain_index]
                ...
                if best_split_info.feature_owner == self.party_id:
                    ...
                    for party_id in self.min_split_info_channs:
                        if best_split_info.feature_owner == party_id:
                            self.min_split_info_channs[party_id].send([best_split_info.feature_idx,
                                                                       best_split_info.max_gain_index],
                                                                      use_pickle=True)
                        else:
                            self.min_split_info_channs[party_id].send([-1, -1],
                                                                      use_pickle=True)
