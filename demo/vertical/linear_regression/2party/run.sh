...
if [ ! -d "${PROJECT_HOME}/dataset/boston_housing_price_vertical/2party" ]; then
  if [ ! -f "${PROJECT_HOME}/python/xfl.py" ]; then
    python "${PROJECT_HOME}/common/dataset/boston_housing_price.py" --mode "vertical" --splits 2 --party "labeled" "1" --norm true
  else
    python "${PROJECT_HOME}/python/common/dataset/boston_housing_price.py" --mode "vertical" --splits 2 --party "labeled" "1" --norm true
  fi
fi
