...
if [ ! -d "${PROJECT_HOME}/dataset/azpro_data_vertical/2party" ]; then
  if [ ! -f "${PROJECT_HOME}/python/xfl.py" ]; then
    python "${PROJECT_HOME}/common/dataset/azpro_data.py" --mode "vertical" --splits 2 --party "labeled" "1" --norm False
  else
    python "${PROJECT_HOME}/python/common/dataset/azpro_data.py" --mode "vertical" --splits 2 --party "labeled" "1" --norm False
  fi
fi
