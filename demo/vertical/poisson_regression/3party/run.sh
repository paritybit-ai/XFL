...
if [ ! -d "${PROJECT_HOME}/dataset/azpro_data_vertical/3party" ]; then
  if [ ! -f "${PROJECT_HOME}/python/xfl.py" ]; then
    python "${PROJECT_HOME}/common/dataset/azpro_data.py" --mode "vertical" --splits 3 --party "labeled" "1" "2" --norm False
  else
    python "${PROJECT_HOME}/python/common/dataset/azpro_data.py" --mode "vertical" --splits 3 --party "labeled" "1" "2" --norm False
  fi
fi
