...
if [ ! -d "${PROJECT_HOME}/dataset/breast_cancer_wisconsin_vertical/3party" ]; then
  if [ ! -f "${PROJECT_HOME}/python/xfl.py" ]; then
    python "${PROJECT_HOME}/common/dataset/breast_cancer_wisconsin.py" --mode "vertical" --splits 3 --party "labeled" "1" "2"
  else
    python "${PROJECT_HOME}/python/common/dataset/breast_cancer_wisconsin.py" --mode "vertical" --splits 3 --party "labeled" "1" "2"
  fi
fi

type="vertical"
operator="logistic_regression"
party="3party"
