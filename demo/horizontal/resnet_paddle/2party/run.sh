...
if [ ! -d "${PROJECT_HOME}/dataset/cifar-10/2party" ]; then
  if [ ! -f "${PROJECT_HOME}/python/xfl.py" ]; then
    python "${PROJECT_HOME}/common/dataset/cifar.py" --splits 2 --party "1" "2" --keep_raw_data
  else
    python "${PROJECT_HOME}/python/common/dataset/cifar.py" --splits 2 --party "1" "2" --keep_raw_data
  fi
fi

type="horizontal"
operator="resnet_paddle"
party="2party"
