...
datapath="${PROJECT_HOME}/dataset"
if [ ! -d "${PROJECT_HOME}/dataset/horizontal_kmeans/3party" ]; then
  if [ ! -f "${PROJECT_HOME}/python/xfl.py" ]; then
    python "${PROJECT_HOME}/common/dataset/h_kmeans.py" --splits 3 
  else
    python "${PROJECT_HOME}/python/common/dataset/h_kmeans.py" --splits 3
  fi
fi
